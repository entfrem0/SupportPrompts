...
- ミリ秒単位の精度を持ち、ワークフロー基盤のタイムゾーン（例：JST +09:00）に従って解釈されますが、扱える情報は「年月日」のみです。
...
#### addDays
- **説明**：指定された○日を加算します。正の値で未来、負の値で過去の日付または日時を得られます。
- **返り値の型**：日付型
- **引数**：`int days`
- **例**：`#q_date?.addDays(3)` → 3日後の日付（例：2025-07-01）

#### addMonths
- **説明**：指定された○月を加算します。年をまたぐ加算や月末処理にも対応します。
- **返り値の型**：日付型
- **引数**：`int months`
...
#### addMinutes
- **説明**：指定された○分を加算します。分単位で日時を加算できます。日付型には使用できません。
- **返り値の型**：日時型
- **引数**：`int minutes`
- **例**：`#q_datetime?.addMinutes(90)` → 90分後の日時（例：2025-06-23 11:30）

#### addHours
- **説明**：指定された○時間を加算します。時刻単位で日時を加算できます。日付型には使用できません。
- **返り値の型**：日時型
- **引数**：`int hours`
...
- **返り値の型**：日時型
- **引数**：`int days`
- **例**：`#q_datetime?.addDays(3)` → 3日後の日付（例：2025-07-01）

#### addMonths
- **説明**：指定された○月を加算します。年をまたぐ加算や月末処理にも対応します。
- **返り値の型**：日時型
- **引数**：`int months`
- **例**：`#q_datetime?.addMonths(2)` → 2ヶ月後の日付（例：2025-08-23）

#### getFirstTimeInDate
- **説明**：該当日の 0時0分0秒 を取得します。日付型の値を日時型に変換する際によく使われます。
- **返り値の型**：日時型
- **引数**：なし
- **例**：`#q_date?time.getFirstTimeInDate()` → 2025-06-23 0:00

#### getFirstTimeInWeek
- **説明**：その週の月曜日の 0時0分0秒 を取得します。週単位の処理や集計の基準日時に使われます。
- **返り値の型**：日時型
- **引数**：なし
- **例**：`#q_datetime?.getFirstTimeInWeek()` → 2025-06-16 0:00
